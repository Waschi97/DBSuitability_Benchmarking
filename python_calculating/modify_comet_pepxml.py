...
min_prec_z = 1 # only analyzer spectra whose precursor charge state is as low as this
max_prec_z = 10 # only analyzer spectra whose precursor charge state is as high as this
min_peptide_length = 0 # min peptide length to analyze
novor_fract = 0.99 # Based on decoy model, the percent of Novor peptides to capture w/ xcorr > fasta xcorr
...
file_name_list = "pep_xml_list.txt" #each line is the name of a pep.xml file to be modified
...
for pepxml_file_name in file_names:
    ...
    print("Number of decoy pairs: " + str(len(decoy_diffs)))
